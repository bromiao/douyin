			...
			>
				<div class="video-item">
					<Videos
						...
						:index="index"
					></Videos>
					<Baberrage ref="barrage"></Baberrage>
				</div>
				<div class="info-bar">
					<InfoBar
				...
				<div class="video-controller">
					...
					<div
						v-show="longPressing && video.playing"
						class="play-rate"
					>
						<i class="icon icon-play-rate"></i>
						<span class="rate-3x">3X</span>
						<span class="rate-tips">快速播放中</span>
					</div>
				</div>
			</swiper-slide>
			<div
...
import Baberrage from './baberrage.vue'
...
export default {
		...
		return {
			...
			swiperOptions: {
				...
				on: {
					...
					tap: (swiper, e) => {
						// 点击右侧按钮区域时，直接退出
						if (this.$refs.rightBar[this.page - 1].$el.contains(e.target)) {
							return false
						}
						// 非长按才响应播放/暂停
					...
					touchStart: () => {
						...
						this.longPressTimer = setTimeout(() => {
							...
							this.$refs.videos[this.page - 1].player.tech_.el_.playbackRate = 3
						}, 400)
					},
					touchMove: () => {
						this.longPressing = false
					...
					// 手指抬起
					touchEnd: () => {
						this.$refs.videos[this.page - 1].player.tech_.el_.playbackRate = 1
			...
			page: 1,
			// 显示播放按钮
			showPlay: !this.$store.getters['global/getIsUserInteractive'],
			// 手指长按中
			longPressing: false,
		}
	},
	computed: {
		...mapGetters('global', ['getIsUserInteractive']),
		// swiper组件实例
		swiper() {
		...
		// 当前视频对象
		video() {
			return this.$refs.videos[this.page - 1]
		},
	},
	created() {
		...
		this.initPlayTimer = setTimeout(() => {
			if (this.getIsUserInteractive) {
				this.playAction(this.page - 1)
				this.$refs.barrage[this.page - 1].barrageIsShow = true
			}
		}, 30)
	...
	destroyed() {
		...
		clearTimeout(this.initPlayTimer)
		this.initPlayTimer = null
	},
	methods: {
		...mapMutations('global', ['setIsUserInteractive']),
		before_unload_page(e) {
			this.setIsUserInteractive(false)
			clearTimeout(this.initPlayTimer)
			this.initPlayTimer = null
		},
		playAction(index) {
			...
			if (index === 0) {
				this.$refs.videos[index].playerOptions.autoplay = true
				this.$refs.videos[index].playerOptions.muted = false
				this.$refs.barrage[index].$refs.baberrage.pause()
			}
			this.showPlay = !this.$refs.videos[index].playing

			if (this.showPlay) {
				this.$refs.barrage[index].$refs.baberrage.pause()
			} else {
				this.$refs.barrage[index].$refs.baberrage.play()
			}
		},
		...
		.swiper-slide {
			...
			.video-controller {
				...
				.icon-play {
					...
					width: 50px;
					height: 50px;
					background: url('~@/assets/images/svg/play.svg') center / cover no-repeat;
				}
				.play-rate {
					display: flex;
					align-items: center;
					...
					.icon-play-rate {
						...
						width: 28px;
						height: 12px;
						background: url('~@/assets/images/svg/play-rate.svg') center / cover no-repeat;
					}
					.rate-3x {
						font-size: 16px;
						font-weight: 700;
